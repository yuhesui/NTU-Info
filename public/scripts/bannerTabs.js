...
export function initBannerTabs() {
  ...
  function positionPanelForTab(tab) {
    const navRect = navRoot.getBoundingClientRect();
    const tabRect = tab.getBoundingClientRect();
    const left = Math.max(0, Math.round(tabRect.left - navRect.left - 12));
    panelsRoot.style.left = `${left}px`;
    panelsRoot.style.right = 'auto';
  }

  function showPanelForTab(tab) {
    positionPanelForTab(tab);
    const panelId = tab.getAttribute('aria-controls');
  ...
  window.addEventListener('resize', () => {
    ...
    if (selectedTab) positionPanelForTab(selectedTab);
  });
